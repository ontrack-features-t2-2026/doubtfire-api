...
| `DF_REDIS_CACHE_URL`        | Redis connection used by the Rails cache and authentication throttling. Production and staging must set this or `DF_REDIS_SIDEKIQ_URL`. | `redis://localhost:6379/0` |
...
| `DF_IMPORT_STUDENTS_WEEKS_BEFORE`     | Weeks before a teaching period that student imports are allowed. Deprecated alias: `DF_IMPORT_STUDENTS_WEEKS_BEFPRE`. | `1`                             |
...
| `DF_INSTITUTION_EMAIL_SENDER` | SMTP-authorised From address for event-notification email. May include a display name. | `noreply@doubtfire.local` |
