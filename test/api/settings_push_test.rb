...
# The front end reads the VAPID public key from /api/settings so it is
# configured in one place instead of being copied into the web repo and going
# stale the first time the keys are rotated. The private key must never leave
# the server.
class SettingsPushTest < ActiveSupport::TestCase
  ...
  # /api/settings is public. The public key may be read before sign-in, the
  # private key never.
  def test_the_private_key_is_not_published_to_an_anonymous_caller
    clear_auth_header
