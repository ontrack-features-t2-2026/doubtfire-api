...
# The push channel actually sends.
#
...
class PushNotificationServiceTest < ActiveSupport::TestCase
  ...
  def test_the_payload_has_the_shape_angulars_service_worker_expects
    ...
    assert_equal 'You have a new comment on a task.', body['body']
  ...
  # The raw message names people, tasks, units and dates. A locked phone must
  # show none of that, whatever the event.
  def test_every_reviewed_event_uses_its_lock_screen_line_and_never_the_message
    sensitive_canary = 'PRIVATE-NAME-AND-SCHEDULE-7429'

    PushNotificationService::LOCK_SCREEN_BODIES.each do |event, expected_body|
      notification = Notification.create!(
        user: @user,
      ...
      payload = PushNotificationService.payload_for(notification)
      body = JSON.parse(payload).dig('notification', 'body')

      assert_equal expected_body, body, event
      # Not only the body. The title, tag and click data must not carry it either.
      assert_not_includes payload, sensitive_canary, event
    end
  end

  def test_the_five_events_that_used_to_leak_detail_now_use_reviewed_lines
    leaks = {
      'task_comment_created' => 'Andrew Cain commented on 1.1P in COS10001.',
      'task_status_changed' => 'Andrew Cain marked 1.1P in COS10001 as Complete.',
      'new_task_available' => '1.1P in COS10001 is now available, due Mon Sep 14.',
      'task_due_soon' => '1.1P in COS10001 is due Mon Sep 14.',
      'task_due_date_changed' => '1.1P in COS10001 is now due Mon Sep 14.'
    }

    leaks.each do |event, message|
      notification = Notification.create!(
        user: @user, notification_type: 'task', event: event, message: message, link: '/notifications'
      )

      payload = PushNotificationService.payload_for(notification)
      body = JSON.parse(payload).dig('notification', 'body')

      assert_equal PushNotificationService::LOCK_SCREEN_BODIES.fetch(event), body
      %w[Andrew 1.1P COS10001 Sep].each { |detail| assert_not_includes body, detail, event }
      %w[Andrew COS10001 Sep].each { |detail| assert_not_includes payload, detail, event }
    end
  end

  def test_an_event_without_a_reviewed_line_gets_the_generic_line
    notification = Notification.create!(
      user: @user,
      notification_type: 'general',
      event: 'not_reviewed_yet',
      message: 'Jane Citizen was moved to Tutorial 3 on Tuesday 9am.',
      link: '/notifications'
    )
    ...
    assert_equal PushNotificationService::GENERIC_LOCK_SCREEN_BODY, body
    %w[Jane Tutorial Tuesday].each { |detail| assert_not_includes payload, detail }
  end

  # The raw message names the outcome and a precise date, and a result and a
  # date are both banned on a lock screen.
  ...
  def test_the_title_is_the_product_name_and_the_body_is_not_the_message
    body = JSON.parse(PushNotificationService.payload_for(@notification))['notification']
  ...
  # A row written before the key validation existed can hold a malformed
