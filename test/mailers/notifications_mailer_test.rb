...
# Every event's mailer templates render without raising, in both
# HTML and text.
#
...
class NotificationsMailerTest < ActionMailer::TestCase
  # event => notification_type, for every event that ships its own templates.
  EVENTS = {}.freeze
  ...
  def test_the_generic_template_renders_the_message_and_link
    notification = FactoryBot.create(
      :notification,
      notification_type: 'general',
      event: 'unwired_event',
      message: 'A generic notification body.',
      link: LINK
    )
    ...
    [mail.html_part.body.to_s, mail.text_part.body.to_s].each do |body|
      assert_includes body, 'A generic notification body.'
