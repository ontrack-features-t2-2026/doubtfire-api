...
class PushNotificationService
  ...
  # Push services reject a payload much over 4KB once encrypted. The bodies
  # below are short, fixed lines, so this is only a backstop.
  MAX_BODY_LENGTH = 400

  # What a locked phone shows. Assume it is face up and visible to someone else,
  # so the body must never carry a person's name, a mark or result, a date, or
  # free-form task, unit, group or comment text.
  #
  # Notification#message can hold all of those, because email and the in-app
  # list show it after sign-in. So the push body never uses it. Each event gets
  # one reviewed line here, and any event without a line gets the generic one.
  # A new event is therefore safe until someone reviews its wording and adds it.
  GENERIC_LOCK_SCREEN_BODY = 'You have a new notification.'.freeze
  LOCK_SCREEN_BODIES = {
    'task_comment_created' => 'You have a new comment on a task.',
    'task_status_changed' => 'A task status changed.',
    'new_task_available' => 'A new task is available.',
    'task_due_soon' => 'A task is due soon.',
    'task_due_date_changed' => 'A task due date changed.',
    ...
    'discussion_request_created' => 'A discussion prompt is ready for you.'
  }.freeze
  ...
  # has to write one.
  #
  # data.link is what the web app reads to decide where to send the user on click.
  #
  ...
  def self.body_for(notification)
    LOCK_SCREEN_BODIES
      .fetch(notification.event.to_s, GENERIC_LOCK_SCREEN_BODY)
