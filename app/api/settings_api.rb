...
class SettingsApi < Grape::API
  ...
  get '/settings' do
    response = {
      ...
      # Web push. The VAPID public key is not a secret, the browser has to send
      # it to the push service to subscribe at all. Serving it here means it is
      # configured in one place instead of being copied into the front end.
      #
